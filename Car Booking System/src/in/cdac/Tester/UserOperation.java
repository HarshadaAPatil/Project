...
public class UserOperation {
	...
	private static boolean isUserExists(String contactno) throws FileNotFoundException {
	...
	public static void userRegistration() throws IOException {
		...
		try {
		if(!isUserExists(person.getPhoneNumber())) {
			...
			System.out.println("\nUser details invalid Please register again!!\n");
	...
	private static boolean validateUser(User person) throws UserNameIsInvalid, UserContactNoException, UserEmailIsInvalid, UserPasswordIsInvalid {
			if(!isNameValid(person.getName())) {
				throw new UserNameIsInvalid("\nUser name should contain only alphabets, and minimum 5 characters.");
			}
			if(!isContactNoValid(person.getPhoneNumber())) {
				throw new UserContactNoException("\nPlease check contact no. and enter 10 digit number.");
			}
			if(!isEmailIDValid(person.getEmailId())) {
				throw new UserEmailIsInvalid("\nEmail is invalid.");
			}
			if(!isPasswordValid(person.getPassword())) {
				throw new UserPasswordIsInvalid("\nPassword is invalid. It should contain 8 character, 1 capital 1 digit 1 small case 1 special character.  ");
			}
...
//			public BookingDetails(int bookingId, String str, String destination, LocalDateTime dateTime, double total) {
	...
	private static void generateInvoice(String str, BookingDetails booking) {
		...
		System.out.println("Amount : "+(float)booking.getFare());
		System.out.println("\n----------------------------------------------------------------\n");
