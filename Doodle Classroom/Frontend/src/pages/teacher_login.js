...
const validationSchema1 = Yup.object({
  password:Yup.string().required('Required').matches(/(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}/,"Password must contain 8 or more characters that are of at least one number, and one uppercase and lowercase letter"),
...
function onSubmit(){
    ...
    makePostRequest("/login/teacher", bodyFormData).then((response) => {
        if (response.data.status === "1") {
            swal("Success","Teacher Login Successfully", "success");
            localStorage.setItem("logintype","teacher");
            localStorage.setItem("institute_id",response.data.data[0].institute_id);
            localStorage.setItem("teacher_id",response.data.data[0].teacher_id);
            localStorage.setItem("userpic",response.data.data[0].t_img);
            localStorage.setItem("Lname",response.data.data[0].teacher_fname+" "+response.data.data[0].teacher_lname);
...
async function sendpassword(e){
  ...
  let bodyFormData = new FormData();
      bodyFormData.append("email",email);
      makePostRequest("/password/send/teacher", bodyFormData).then((response) => {
          if (response.data.status === "1") {
...
async function reset_password(){
  ...
  makePostRequest("/update/password/teacher", bodyFormData).then((response) => {
      if (response.data.status === "1") {
          swal("Success","Teacher Verify Successfully", "success");
         handleClose9();
        }else if(response.data.status === "3"){  
