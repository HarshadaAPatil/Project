...
import MoreVertIcon from '@mui/icons-material/DeleteForever';
...
function Group() {
   ...
   async function deleteMsg(mid){
    let bodyFormData = new FormData();
    makePostRequest("/delete/message/"+mid, bodyFormData).then((response) => {
    if (response.data.status === "1") {
       swal("Success",response.data.message, "success"); 
       view_chat();
    }else{  
...
<Grid container spacing={4} className="listingItem topSpace">
<Grid item xs={12} md={3} >
<Paper className="sectionnBox" >
    <h2 className="noTopMar">Group Members ({studcount})</h2>
...
<li className="me ">
<section className="message bg-success-subtle" style={{color:"black"}}>
<div className="btnaction"><MoreVertIcon onClick={()=>deleteMsg(m.m_id)}/></div>
   <div className="user_info">
       <div className="avatar"><img src={UserPic} alt="avatar"/></div>
       <span className="nickname">{m.m_name}</span>
       
   </div>
   
   <div className="message_text">
    {(m.is_deleted==0)?<>{m.msg}</>:<i>This message was deleted</i>}
   </div>
   <span className="time11">{moment(m.msg_date).format("MMM Do YY")}</span>
   <span className="time">{dateFormat(m.msg_date,"h:MM TT")}</span>
</section>
</li>
    :
    <li className="you">
        <section className="message bg-primary-subtle" style={{color:"black"}}>
        {/* <div className="btnaction"><MoreVertIcon onClick={()=>deleteMsg(m.m_id)}/></div> */}
   <div className="user_info">
       <div className="avatar"><img src={UserPic} alt="avatar"/></div>
       <span className="nickname">{m.m_name}</span>
