...
//const DatabaseConn ="mysql://root:@localhost:3306/pizza-system";
const DatabaseConn = "postgres://postgres:harshada@localhost:5432/backendv2";
...
export class DbManager {
 ...
 public async insert(table: string,columns: string,value: string,id: string) 
  {
    ...
    let lastid
    try{lastid = await this.db.query(
      "insert into " + table + "(" + columns + ") values(" + value + ") returning "+id
    );}catch(err){
      console.log(err)
      console.log(
 ...
 public async findfieldpagi(field, table, where,limit,offset) {
    ...
    //SELECT * FROM users WHERE fname LIKE 'harshada%'
    let result = await this.db.query(
