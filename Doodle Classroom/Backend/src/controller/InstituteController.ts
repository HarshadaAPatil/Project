...
//Group    
    ...
    this.router.get("/search/group/stud", this.search_group_stud);
  ...
  public search_group = async (req: Request, res: Response) => {
    ...
    var id=req.query.id;
    let where = ` where iid=${id} and  LOWER(groups.group_name) LIKE LOWER('%${q}%') `;
  ...
  public search_group_stud = async (req: Request, res: Response) => {
    res.header("Access-Control-Allow-Origin", "*");
    res.header(
      "Access-Control-Allow-Headers",
      "Cache-Control, Pragma, Origin, Authorization, Content-Type, X-Requested-With"
    ...
    let where = ` LEFT JOIN studgrp sg ON sg.instituteid=g.iid where g.iid=${iid} and sg.studentid=${sid} and LOWER(groups.group_name) LIKE LOWER('%${q}%') `;
  ...
  public login_user = async (req: Request, res: Response) => {
    ...
    let field = ' *';
    let where = ` where del_flag='0' and institute_email='${email}'`;
      //  and institute_password='${password}'
    try {
      let result = await this.ParentService.findfield(field,"institute",where);
      
      if (result.rowCount > 0) {
        var top = result.rows;
        const isPasswordMatch = EncryptionUtil.compare(password,result.rows[0].institute_password);
        if(isPasswordMatch) 
        {      
        ...
        {
          res.json({ status: "0", message: "Login Failed, Invalid username or password" });
        }
      } else {
        res.json({ status: "0", message: "No data found" });
      }
  ...
  public addinstitute = async (req: Request, res: Response) => {
    ...
    let hashedPassword = EncryptionUtil.encrypt(req.body.institute_password, salt);
    var Profile
    var uploadPath
    console.log(req);
    Profile = req.files.Profile;
    let img=CommonUtils.generateUniqueID()+"-"+Profile.name;
    let path="D:/Breeze Chat/final/end/Breeze Chat/f2/public";
    // D:\Breeze Chat\final\end\Breeze Chat\f2\public
    // D:\Breeze Chat\final\end\Breeze Chat\frontend\public
    // D:/CDAC/Final Project/Breez Chat/frontend/public
    uploadPath = path + '/uploads/' +img ;
    
    console.log("Fileeeeeeeeeeeeeeee Nameeeeeeeeeeeeeeeeee"+Profile.name)
    ...
    const val= `'${req.body.institute_name}','${req.body.institute_email}','${hashedPassword}','${req.body.institute_contact}','0','${img}'`;
